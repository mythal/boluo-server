...
pub async fn alloc_new_pos<T: Querist>(
    ...
    let in_cache: bool = cache
        .inner
        .get::<_, Option<i64>>(&max_pos_key)
        .await?
        .is_some();

    if !in_cache {
        // if not present, initialize it
        let initial_pos = crate::messages::Message::max_pos(db, &channel_id)
            .await
            .ceil();
        let _: () = cache
            .inner
            .set_nx(&max_pos_key, initial_pos as i64 + 1)
            .await?;
    }
...
pub async fn finished(
    ...
    cache
        .inner
        .del(create_pos_key(channel_id, message_id))
        .await
}
