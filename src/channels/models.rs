...
impl Channel {
    ...
    pub async fn get_by_id<T: Querist>(db: &mut T, id: &Uuid) -> Result<Option<Channel>, DbError> {
        let result = db
            .query_one(include_str!("sql/fetch_channel.sql"), &[&id])
            .await;
        inner_result_map(result, |row| row.try_get(0))
    }

    pub async fn get_by_name<T: Querist>(
        db: &mut T,
        space_id: Uuid,
        name: &str,
        ...
        let result = db
            .query_one(
                include_str!("sql/get_channel_by_name.sql"),
                &[&space_id, &name],
            )
            .await;
        inner_result_map(result, |row| row.try_get(0))
    ...
    pub async fn get_with_space<T: Querist>(
        db: &mut T,
        id: &Uuid,
    ) -> Result<Option<(Channel, Space)>, DbError> {
        let row = db
    ...
    pub async fn get_by_space<T: Querist>(
        ...
        let rows = db
            .query(include_str!("sql/get_by_space.sql"), &[space_id])
            .await?;
        Ok(rows.into_iter().map(|row| row.get(0)).collect())
    }

    pub async fn delete<T: Querist>(db: &mut T, id: &Uuid) -> Result<u64, DbError> {
        db.execute(include_str!("sql/delete_channel.sql"), &[id])
            .await
    }
    ...
    pub async fn max_pos<T: Querist>(db: &mut T) -> Result<Vec<(Uuid, f64)>, DbError> {
        let rows = db
            .query(include_str!("sql/channel_max_pos.sql"), &[])
            .await?;
    ...
    pub async fn get_by_user<T: Querist>(
        db: &mut T,
        user_id: Uuid,
    ) -> Result<Vec<ChannelWithMember>, DbError> {
        let rows = db
...
impl ChannelMember {
    ...
    pub async fn get_color_list<T: Querist>(
        db: &mut T,
        channel_id: &Uuid,
    ) -> Result<HashMap<Uuid, String>, DbError> {
        let rows = db
            .query(include_str!("sql/get_color_list.sql"), &[channel_id])
            .await?;
        Ok(rows
            .into_iter()
            .map(|row| (row.get(0), row.get(1)))
    ...
    pub async fn get_by_space<T: Querist>(
        ...
        let rows = db
            .query(
                include_str!("sql/get_channel_member_list_by_space.sql"),
                &[space_id],
            )
            .await?;
    ...
    pub async fn is_master<T: Querist>(
        db: &mut T,
        user_id: &Uuid,
        channel_id: &Uuid,
    ) -> Result<bool, DbError> {
    ...
    pub async fn get_with_space_member<T: Querist>(
        ...
        let mut rows = db
            .query(
                include_str!("sql/get_with_space_member.sql"),
                &[user_id, channel_id],
            )
            .await?;
    ...
    pub async fn remove_user<T: Querist>(
        ...
        db.execute(
            include_str!("sql/remove_user_from_channel.sql"),
            &[user_id, channel_id],
        )
        .await
    }
    ...
    pub async fn remove_user_by_space<T: Querist>(
        ...
        let channels = db
            .query(
                include_str!("sql/remove_user_by_space.sql"),
                &[user_id, space_id],
            )
            .await?
            .into_iter()
...
async fn channels_test() -> Result<(), crate::error::AppError> {
    ...
    let new_name = "深水城水很深";
    let channel_edited = Channel::edit(
        db,
        &channel.id,
        Some(new_name),
        None,
    ...
    assert!(Channel::get_by_name(db, space.id, "Madoka")
        .await?
        .is_none());
    ...
    let member_altered = ChannelMember::get(db, &user.id, &channel.id)
        .await?
        .unwrap();
    ...
    assert_eq!(
        ChannelMember::get_by_channel(db, &channel.id, false)
            .await?
            .len(),
        0
    );
    ...
    assert!(ChannelMember::get(db, &user.id, &channel.id)
        .await?
        .is_none());
    assert!(ChannelMember::get(db, &user.id, &channel_2.id)
        .await?
        .is_none());
